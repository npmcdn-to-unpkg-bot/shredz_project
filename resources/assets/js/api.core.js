/**
 * FG-REST-API: Core Module Definition
...
!(function (window, undefined) {
  ...
  function merge(/* [deep=false], target, obj1, [objN] */) {
    var args = Array.prototype.slice.call(arguments),
      index = -1,
      deep = (args.length > 1) && (typeof args[0] === 'boolean') && args.shift(),
   ...
   */
  function ShredzAPI(options) {
    var $instance = this;
    var $static = $instance && $instance.constructor || ShredzAPI;
  ...
  // Expose Constants
  ShredzAPI.METHOD_GET = METHOD_GET;
  ShredzAPI.METHOD_POST = METHOD_POST;
  ShredzAPI.METHOD_PUT = METHOD_PUT;
  ShredzAPI.METHOD_DELETE = METHOD_DELETE;

  // Publish Public properties and methods
  ShredzAPI.request = requestApi;
  ShredzAPI.routes = function getRoutes(name) {
    return name ? _routes[name] : _routes;
  }

  ShredzAPI.route = function registerRoute(name, requestOptions) {
    ...
    ShredzAPI[METHOD_GET.toLowerCase() + name.upperCaseFirst()] = route[METHOD_GET.toLowerCase()];
    ShredzAPI[METHOD_POST.toLowerCase() + name.upperCaseFirst()] = route[METHOD_POST.toLowerCase()];
    ShredzAPI[METHOD_PUT.toLowerCase() + name.upperCaseFirst()] = route[METHOD_PUT.toLowerCase()];
    ShredzAPI[METHOD_DELETE.toLowerCase() + name.upperCaseFirst()] = route[METHOD_DELETE.toLowerCase()];

    return ShredzAPI;
  };

  ShredzAPI.options = function getSetOptions(options) {
    if (options) {
  ...
  ShredzAPI.factories = function getFactory(name) {
    return name ? _factories[name] : _factories;
  };

  ShredzAPI.factory = function registerFactory(name, Model) {
    if (!/^([A-Z][a-z]*)+$/.test(name)) {
      throw new Error('Factory name must begin with an uppercase');
    }

    ShredzAPI[name + 'Factory'] = _factories[name] = new Factory(Model);
  ...
  // Export module
  window.ShredzAPI = ShredzAPI;
  // Allow for AMD loader
  window.define && window.define([], ShredzAPI);
})(window);
